...
export function buildSidewalkElevationSvg(design: StreateryDesign): string {
  ...
  el.push(
    `<rect x="0" y="${yOf(deckTopFt)}" width="${L}" height="0.12" fill="#e7e5e4" stroke="#1c1917" stroke-width="0.06" />`,
  );
  ...
  // street view uses for the entry.
  ...
  // ---------- Posts (near side, mirrored stations) ----------
  ...
  for (const post of design.posts) {
    ...
    const topFt = roofed ? highEdgeFt - beamDepthFt : enclosureTopFt;
    const x = Math.min(
  ...
  // ---------- Roof: high-edge beam band + fascia band ----------
