    ...
    <div className="space-y-6">
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <input
          ...
          className="w-full px-3 py-2 text-sm border border-stone-300 rounded-md focus:outline-none focus:ring-2 focus:ring-stone-400 focus:border-stone-400 disabled:bg-stone-100"
        />
        <button
          type="submit"
          disabled={isSubmitting || value.trim().length === 0}
          className="w-full px-4 py-2 text-sm font-medium text-white bg-stone-800 rounded-md hover:bg-stone-900 disabled:bg-stone-300 disabled:cursor-not-allowed transition-colors"
        >
          ...
          {QUICK_PICKS.map((pick) => (
            <li key={pick.address}>
              <button
                ...
                disabled={isSubmitting}
                className="w-full text-left px-3 py-2 text-sm rounded-md hover:bg-stone-100 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
